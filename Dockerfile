...
ENV VIRTUAL_ENV /opt/arazzo-ai-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/arazzo-ai/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/arazzo-ai/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/arazzo_ai/ && touch src/arazzo_ai/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/arazzo-ai/
...
# Expose the app.
ENTRYPOINT ["/opt/arazzo-ai-env/bin/arazzo-ai"]
CMD []
